...
#
#   方式（旧CLI版からの刷新）:
#     - カメラ＋YOLO＋トラッカーを本番(main_5goki_JP_v3)と同条件で回す。
...
# main_5goki_JP_v3.py の update_camera_delays と同じ値（確定タイミング再現のため合わせる）
CAMERA_DELAYS = {"cam_under": 1.922, "cam_inside": 2.015}

# 本番のループ周期（main の QTimer は 50ms = 20Hz）
LOOP_INTERVAL_MS = 50
...
class CalibrationWindow(QMainWindow):
    def __init__(self):
        ...
        # 検知ループ（main.update_video_feeds 相当）
        self.timer = QTimer(self)
        self.timer.timeout.connect(self._update_loop)
        self.timer.start(LOOP_INTERVAL_MS)
    ...
    def _update_loop(self):
        for c in self.controllers:
            ...
            if self.running and self.detector is not None:
                try:
                    annotated, _result, finalized = self.detector.evaluate_frame(frame, c.name)
                except Exception as e:
                    print(f"!! evaluate_frame でエラー（このフレームを無視）: {e}")
                    continue
                ...
                display = frame
            self._show_frame(c.name, display)
    ...
    def _show_frame(self, cam_name, frame):
