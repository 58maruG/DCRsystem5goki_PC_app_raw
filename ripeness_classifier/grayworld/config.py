"""設定の既定値・定数・JSON入出力（グレーワールド正規化版・未熟マスク・過熟マスク占有率モデル）。"""
...
_CONFIG_PATH = _JSON_DIR / "hsv_ripeness_config_grayworld.json"
...
# グレーワールド正規化: True で各画像の色温度差を補正してから判定・表示する。
DEFAULT_GRAYWORLD = True
...
def load_config() -> dict:
    if _CONFIG_PATH.exists():
        ...
        # 旧モデル（赤色/深赤マスク・threshold・未使用の残存キー）は不要なので取り除く
        cfg.pop("red", None)
        cfg.pop("deepred", None)
        cfg.pop("threshold", None)
        cfg.pop("v_overripe", None)
        cfg.pop("overripe_red", None)
        cfg.setdefault("grayworld",    DEFAULT_GRAYWORLD)
        return cfg
    return {
