...
STEMC_STRICT_THRESHOLD = 0.85
CRACK_STRICT_THRESHOLD = 0.85
BIRDD_STRICT_THRESHOLD = 0.85
TWIN_STRICT_THRESHOLD  = 0.9
BTWIN_STRICT_THRESHOLD  = 0.9
MALF_STRICT_THRESHOLD  = 0.9
UNRIPE_STRICT_THRESHOLD = 0.9
...
class ImageProcessor:
    ...
    def get_target_info(frame, cam_name: str | None = None) -> dict | None:
        ...
        search_paths = []
        if cam_name:
            search_paths.append(os.path.join("hsv_json", f"hsv_config_{cam_name}.json"))
        #search_paths.append(os.path.join("hsv_json", "hsv_common_config.json"))

        # [B] 共通JSONのみ使う場合はこちらをコメントインし、[A] ブロックをコメントアウト
        #search_paths = [os.path.join("hsv_json", "hsv_common_config.json")]
...
class YoloDetector:
    ...
    def _resolve_best_result(self, detections: list) -> YoloResult | None:
        ...
        (1) まず「複数カメラ一致による確定ルール」を判定する。成立すれば、
            全体の最大信頼度によらずそのクラスを確定し、確定クラスの
            最高信頼度の検出（GUI/CSVに返す対象）を返す。
        ...
        # ============================================================
        # (1) 確定ルール: 指定クラスが複数カメラで検出されたら最大信頼度によらず確定
        # ============================================================
        ...
        for d in detections:
            if d.label_name == "healthy" or d.label_name == "None":
                continue
            if d.label_name == "stemcrack":
                if d.confidence > STEMC_STRICT_THRESHOLD:
                    other_damaged_list.append(d)
            elif d.label_name == "crack":
                if d.confidence > CRACK_STRICT_THRESHOLD:
                    other_damaged_list.append(d)
            elif d.label_name == "birddamage":
                if d.confidence > BIRDD_STRICT_THRESHOLD:
                    other_damaged_list.append(d)
            elif d.label_name == "twin":
                if d.confidence >= TWIN_STRICT_THRESHOLD:
                    other_damaged_list.append(d)
            elif d.label_name == "blacktwin":
                if d.confidence >= BTWIN_STRICT_THRESHOLD:
                    other_damaged_list.append(d)
            elif d.label_name == "malformation":
                if d.confidence >= MALF_STRICT_THRESHOLD:
                    other_damaged_list.append(d)
            elif d.label_name == "unripe":
                if d.confidence >= UNRIPE_STRICT_THRESHOLD:
                    unripe_list.append(d)
            else:
                # mold, stemcrack, birddamage, blacktwin 等
                # 非確定 blacktwin もここで通常の病害として扱う:
                #   ・healthy と共存 → 被害優先で blacktwin（other_damaged が healthy より上）
                #   ・他病害と共存 → other_damaged 内の最大信頼度で勝負
                other_damaged_list.append(d)
